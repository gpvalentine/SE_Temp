...
For the linear regression, we omitted observations where air temperatures were less than 0$^\circ$C. We then fit weekly mean water temperature $T_W$ ($^\circ$ C) at site $i = 1,...,203$ and week $t = 1,...,T$ as a function of weekly mean air temperature $T_A$ ($^\circ$C) with
...
\noindent where $\beta_i$ is modeled as
...
\begin{equation}
  \begin{aligned}
    ...
    \bm{\theta} \sim \text{normal}(\bm{0}, \bm{100}), \\
...
(ref:min-max-interpolated-map-plot-cap) a) Minimum and b) maximum weekly stream temperatures at 8,695 stream segments suitable as brook trout habitat. Stream segments were obtained through EcoSHEDS (www.usgs.gov/apps/ecosheds) and the Eastern Brook Trout Joint Venture [@ebtjv2006].  Minimum and maximum stream temperatures were spatially interpolated from measured temperatures while accounting for latitude, longitude, and elevation.

```{r min-max-interpolated-map-plot, fig.cap="(ref:min-max-interpolated-map-plot-cap)", warning=FALSE, out.width="100%", out.height="100%"}
...
corrplot::corrplot(cor(nonlinear_betas_corr.table), method = 'number')
```
