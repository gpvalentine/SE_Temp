...
\noindent where $\sigma \sim \text{uniform}(0,10)$ and $g(\alpha_i, \beta_i, \text{T}_{Ai,t})$ is a linear function of observed air temperature $T_A$ ($^\circ$C) and the top five principle components of landscape variables:
...
\noindent $\beta_i$, the slope of the linear air-water temperature relationship at each site, arises from $\beta_i \sim \text{normal}(g(\theta_l, \text{PCA}_{l,i}), \sigma^2_\beta)$ where $\sigma_\beta \sim \text{uniform}(0,10)$ and $g(\theta_l, \text{PCA}_{l,i})$ for $l$ in $1,...,6$ is the linear function

\begin{equation}
  g(\theta_l, \text{PCA}_{l,i}) = \theta_1\text{PCA}_{1,i} + \theta_2\text{PCA}_{2,i} + \theta_3\text{PCA}_{3,i} + \theta_4\text{PCA}_{4,i} + \theta_5\text{PCA}_{5,i} + \theta_6\text{PCA}_{6,i}.
  (\#eq:PCALinear)
\end{equation}

\noindent $\bm{\theta}$, the contribution of each principal component to thermal sensitivity, arises from $\theta_l \sim \text{normal}(0, 100)$.

We also implemented a nonlinear model to relate observed air and water temperatures. We consider this function due to the established nonlinear behaviors of water temperature at high and low air temperatures [@mohseni1998a]. Following @mohseni1998a, we replace Eq. \@ref(eq:Linear) with

\begin{equation}
  g(\epsilon_i, \zeta_i, \phi_i, \kappa_i, \text{T}_{Ai,t}) = \epsilon_i + \frac{\zeta_i - \epsilon_i}{1 + e^{\phi_i(\kappa_i - \text{T}_{Ai,t})}},
  (\#eq:MohseniModel)
\end{equation}

\noindent where $\epsilon_i$ represents the minimum stream temperature ($^\circ$C) at site $i$, $\zeta_i$ the maximum stream temperature ($^\circ$C), $\kappa_i$ the air temperature at the inflection point of the function ($^\circ$C), and $\phi_i$ a measure of the slope of the function at $\kappa_i$ ($^\circ$C^$^{-1}$). Similar to $\beta_i$ in \@ref(eq:Linear), $\phi_i$ arises from $\phi_i \sim \text{normal}(g(\theta_l, \text{PCA}_{l,i}), \sigma^2_\phi)$ where $\sigma_\phi \sim \text{uniform}(0,10)$ and $g(\theta_l, \text{PCA}_{l,i})$ for $l$ in $1,...,6$ is \@ref(eq:PCALinear).  Following @mohseni1998a, the maximum slope of the air-water temperature relationship ($\beta_i$) at site $i$ can be calculated from the exponent $\phi_i$ using the equation
...
\noindent We visually evaluated spatial structure in thermal sensitivity by plotting semivariograms of model $\bm{\beta}$ using the 'geoR' package in R [@ribeirojr2022].
...
We predicted thermal sensitivity at unsampled brook trout habitat throughout the southeast USA. In Section \@ref(principal-components-analysis), we calculated principal components for nearly 9,000 stream segments of known brook trout habitat. Using these principal components and $\bm{\theta}$ values calculated from \@ref(eq:PCALinear), we calculated $\phi_i$ for each segment. From there, we used \@ref(eq:CalcSlope) to estimate thermal sensitivity at these unsampled sites. We used posterior medians of $\bm{\zeta}$ and $\bm{\epsilon}$ from the weekly nonlinear model (\@ref(eq:MohseniModel)). We defined preferred thermal habitat as the lowest 25th percentile of these thermal sensitivity values.
